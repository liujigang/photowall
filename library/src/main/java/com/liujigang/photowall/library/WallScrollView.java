...
public class WallScrollView extends ScrollView implements OnTouchListener {
	...
	private static Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			WallScrollView wallScrollView = (WallScrollView) msg.obj;
			int scrollY = wallScrollView.getScrollY();
			// 如果当前的滚动位置和上次相同，表示已停止滚动
			if (scrollY == lastScrollY) {
				// 当滚动的最底部，并且当前没有正在下载的任务时，开始加载下一页的图片
				if (scrollViewHeight + scrollY >= scrollLayout.getHeight()
						&& taskCollection.isEmpty()) {
					wallScrollView.loadMoreImages();
				}
				wallScrollView.checkVisibility();
			} else {
				lastScrollY = scrollY;
				Message message = Message.obtain();
				message.obj = wallScrollView;
				// 5毫秒后再次对滚动位置进行判断
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && !loadOnce) {
            ...
            firstColumn= (LinearLayout) scrollLayout.getChildAt(0);
            secondColumn= (LinearLayout) scrollLayout.getChildAt(1);
            thirdColumn= (LinearLayout) scrollLayout.getChildAt(2);
	...
	class LoadImageTask extends AsyncTask<String, Void, Bitmap> {
		...
		private void downloadImage(String imageUrl) {
			...
			try {
				...
				HttpGet get = new HttpGet(Uri.encode(imageUrl,
						"@#&=*+-_.,:!?()/~'%"));
